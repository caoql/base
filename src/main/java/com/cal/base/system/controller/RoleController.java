...
public class RoleController extends BaseController {
    ...
    @RequiresRoles("admin")
 	...
 	@ResponseBody
 	public Object delete(@PathVariable("id") String roleId) {
 		boolean flag = roleService.deleteByPrimaryKey(roleId);
 		if (!flag) {
	 ...
	 * @return
	 */
	@GetMapping("/editpage/{roleId}")
	public String editPage(Model model, @PathVariable String roleId) {
		RolePO role = roleService.queryRole(roleId);
	...
	@PostMapping("/edit")
	@ResponseBody
	public Object updateRole(@Validated RoleVO editVo, BindingResult result) {
		logger.debug("post请求编辑的角色数据是:" + editVo);
		// 框架层面的校验
		if (result.hasErrors()) {
			List<ObjectError> allErrors = result.getAllErrors();
			StringBuffer sb = new StringBuffer();
		...
		boolean flag = roleService.updateRole(editVo);
		if (!flag) {
